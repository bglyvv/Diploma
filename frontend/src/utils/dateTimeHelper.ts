...
export const toLocalUIDateTime = (isoDate: string): string => {
    const localDateTime = fromISODate(isoDate);
    const timeZone = Intl.DateTimeFormat().resolvedOptions().timeZone;
    const offsetMinutes = DateTime.local().setZone(timeZone).offset;
    const offsetHours = offsetMinutes / 60;
    const adjustedDateTime = localDateTime.plus({ hours: offsetHours });
    const formattedDateTime = toUIDate(adjustedDateTime, true);
